/*
  This is a SAMPLE FILE to get you started.
  Please, follow the project instructions to complete the tasks.
*/

document.addEventListener('DOMContentLoaded', () => {
    // Detect which page is loaded by checking an element or page title
    const path = window.location.pathname;
  
    if (path.endsWith('index.html') || path === '/' || path === '') {
      initIndexPage();
    } else if (path.endsWith('place.html')) {
      initPlacePage();
    } else if (path.endsWith('add_review.html')) {
      initAddReviewPage();
    } else if (path.endsWith('login.html')) {
      initLoginPage();
    }
  });
  
  /**
   * Initialize the index.html page
   * Populate the list of places and price filter options dynamically
   */
  function initIndexPage() {
    // Example data, you should replace it with data from API/backend
    const places = [
      { id: 1, name: 'Cozy Apartment', price: 80 },
      { id: 2, name: 'Beach House', price: 150 },
      { id: 3, name: 'Mountain Cabin', price: 120 },
    ];
  
    const placesList = document.getElementById('places-list');
    const priceFilter = document.getElementById('price-filter');
  
    // Populate price filter options dynamically (e.g., max prices)
    const prices = [...new Set(places.map(p => p.price))].sort((a, b) => a - b);
    prices.forEach(price => {
      const option = document.createElement('option');
      option.value = price;
      option.textContent = `$${price}`;
      priceFilter.appendChild(option);
    });
  
    // Display all places initially
    displayPlaces(places, placesList);
  
    // Filter places on price change
    priceFilter.addEventListener('change', () => {
      const maxPrice = Number(priceFilter.value);
      const filtered = places.filter(p => p.price <= maxPrice);
      displayPlaces(filtered, placesList);
    });
  }
  
  /**
   * Helper function to display place cards on index page
   */
  function displayPlaces(places, container) {
    container.innerHTML = ''; // clear existing
  
    places.forEach(place => {
      const card = document.createElement('div');
      card.className = 'place-card';
      card.style.margin = '20px';
      card.style.padding = '10px';
      card.style.border = '1px solid #ddd';
      card.style.borderRadius = '10px';
  
      card.innerHTML = `
        <h3>${place.name}</h3>
        <p>Price per night: $${place.price}</p>
        <button class="details-button" data-id="${place.id}">View Details</button>
      `;
  
      container.appendChild(card);
    });
  
    // Add event listeners on all detail buttons
    container.querySelectorAll('.details-button').forEach(button => {
      button.addEventListener('click', (e) => {
        const placeId = e.target.getAttribute('data-id');
        // Redirect to place details page (could add query param)
        window.location.href = `place.html?id=${placeId}`;
      });
    });
  }
  
  /**
   * Initialize the place.html page
   * Populate place details and reviews dynamically
   */
  function initPlacePage() {
    const placeDetailsSection = document.getElementById('place-details');
    const reviewsSection = document.getElementById('reviews');
    const addReviewSection = document.getElementById('add-review');
  
    // For demo, use static data or get place id from URL and fetch details
    const placeId = new URLSearchParams(window.location.search).get('id') || 1;
    const place = {
      id: placeId,
      name: 'Cozy Apartment',
      host: 'John Doe',
      price: 80,
      description: 'A nice and cozy apartment in the city center.',
      amenities: ['WiFi', 'Air Conditioning', 'Kitchen'],
    };
  
    const reviews = [
      { id: 1, user: 'Alice', comment: 'Great place!', rating: 5 },
      { id: 2, user: 'Bob', comment: 'Very comfortable.', rating: 4 },
    ];
  
    // Populate place details
    placeDetailsSection.className = 'place-details place-info';
    placeDetailsSection.style.margin = '20px';
    placeDetailsSection.style.padding = '10px';
    placeDetailsSection.style.border = '1px solid #ddd';
    placeDetailsSection.style.borderRadius = '10px';
  
    placeDetailsSection.innerHTML = `
      <h2>${place.name}</h2>
      <p><strong>Host:</strong> ${place.host}</p>
      <p><strong>Price per night:</strong> $${place.price}</p>
      <p>${place.description}</p>
      <p><strong>Amenities:</strong> ${place.amenities.join(', ')}</p>
    `;
  
    // Populate reviews
    reviewsSection.innerHTML = '';
    reviews.forEach(review => {
      const card = document.createElement('div');
      card.className = 'review-card';
      card.style.margin = '20px';
      card.style.padding = '10px';
      card.style.border = '1px solid #ddd';
      card.style.borderRadius = '10px';
  
      card.innerHTML = `
        <p><strong>${review.user}:</strong> ${review.comment}</p>
        <p>Rating: ${review.rating} / 5</p>
      `;
  
      reviewsSection.appendChild(card);
    });
  
    // Check if user is logged in (fake check here, replace with real auth)
    const loggedIn = Boolean(localStorage.getItem('userLoggedIn'));
  
    if (loggedIn) {
      addReviewSection.style.display = 'block';
    } else {
      addReviewSection.style.display = 'none';
    }
  
    // Handle add review form submission
    const reviewForm = document.getElementById('review-form');
    if (reviewForm) {
      reviewForm.addEventListener('submit', (e) => {
        e.preventDefault();
        // Collect review data, send to backend or update UI
        alert('Review submitted! (functionality to be implemented)');
        reviewForm.reset();
      });
    }
  }
  
  /**
   * Initialize the add_review.html page
   * Possibly verify user authentication and handle form
   */
  function initAddReviewPage() {
    // You can get place ID from URL params if needed
    const placeId = new URLSearchParams(window.location.search).get('id');
  
    // Check if user is logged in (fake check)
    const loggedIn = Boolean(localStorage.getItem('userLoggedIn'));
  
    if (!loggedIn) {
      alert('You must be logged in to add a review.');
      window.location.href = 'login.html';
      return;
    }
  
    const reviewForm = document.getElementById('review-form');
    if (reviewForm) {
      reviewForm.addEventListener('submit', (e) => {
        e.preventDefault();
        // Implement review submission logic here
        alert('Review submitted for place id: ' + placeId);
        reviewForm.reset();
      });
    }
  }
  
  /**
   * Initialize the login.html page
   * Handle login form submission and save login state
   */
  function initLoginPage() {
    const loginForm = document.getElementById('login-form');
    if (loginForm) {
      loginForm.addEventListener('submit', (e) => {
        e.preventDefault();
  
        const email = loginForm.email.value.trim();
        const password = loginForm.password.value.trim();
  
        // For demo, accept any non-empty credentials
        if (email && password) {
          // Save login state (replace with real auth)
          localStorage.setItem('userLoggedIn', 'true');
          alert('Login successful!');
          window.location.href = 'index.html';
        } else {
          alert('Please enter email and password.');
        }
      });
    }
  }
